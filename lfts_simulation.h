...
class lfts_simulation {
    ...
    public:
        lfts_simulation(std::string inputFile, int TpB=512) {
            ...
            dbc_ = new diblock(NA_, N_-NA_, M_, Mk_, m_, L_, TpB_);

            // Create a new anderson mixing class
            AM_ = new anderson(M_, 10, TpB_);
            ...
            Sk_ = new strFunc(m_, L_, M_, Mk_, C_*V_, chi_b_);
        }
        ...
        void equilibrate() {
            ...
            for (it=1; it<=equil_its_; it++) {
                ...
                langevin(w_gpu_, chi_b_, sigma_, dt_, &noise_gpu_new_, &noise_gpu_prev_);
    ...
    private:
        ...
        void langevin(double* w_gpu, double chi_b, double sigma, double dt, double **noise_gpu_new, double **noise_gpu_prev)
        {
            double *ptr_tmp;

            // Create new random noise on the GPU for the call to langevin()
            curandGenerateNormalDouble(RNG_, *noise_gpu_new, M_, 0.0, sigma_);
